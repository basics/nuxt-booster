...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(moduleOptions, nuxt) {
    ...
    if (moduleOptions.optimizeSSR) {
      optimizeSSR(moduleOptions.optimizeSSR, nuxt);
    } else {
      logger.warn(
...
async function addBuildTemplates(nuxt: Nuxt, options: ModuleOptions) {
  ...
  (['client', 'server'] as ['client', 'server']).forEach(mode => {
    addPluginTemplate({
      getContents: () => {
...
async function addModules(nuxt: Nuxt, moduleOptions: ModuleOptions) {
