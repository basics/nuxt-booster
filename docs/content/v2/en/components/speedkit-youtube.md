...
The `SpeedkitYoutube`is used to initialise Youtube videos with Youtube-API only when they are in the visible viewport.
...
Option `playsinline` is always set, `mute` is set automatically for touch device devices. This is important for autoplay on mobile devices.
