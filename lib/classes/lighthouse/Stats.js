...
export class LighthouseStats {
  ...
  getStateColorByMetric (metric) {
    switch (this.state) {
      ...
      case FAIL:
        return STATECOLORS[String(FAIL)];
      default:
        return STATECOLORS[String(PENDING)];
    }
  }
}
...
function getColorByScore (score) {
  if (score >= 0.9) {
    return STATECOLORS[String(READY)].pass;
  } else if (score >= 0.5) {
    return STATECOLORS[String(READY)].average;
  } else {
    return STATECOLORS[String(READY)].fail;
  }
}
