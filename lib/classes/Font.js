...
export default class Font {
  constructor (family, { style = 'normal', weight = 400, src }, critical = new Critical(), selector = [DEFAULT_SELECTOR], usedSelectors) {
    this.family = family
    this.style = style
    this.weight = weight
    this.src = src
    ...
    this.usedSelectors = usedSelectors || this.selectors
  }
  ...
  addSelector (selector) {
    const splittedSelectors = splitSelector(formatSelector(selector))
    splittedSelectors.forEach(selector => this.usedSelectors.add(selector))
    return new Font(this.family, { style: this.style, weight: this.weight, src: this.src }, this.critical, splittedSelectors, this.usedSelectors)
  }
