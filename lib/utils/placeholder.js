export function createPlaceholder (sources, getResources, critical) {
  return Promise.all(sources.map(async (source) => {
    ...
    return Object.assign(createSourceDescription(url, getMinMaxSize(sizes)), { dataURI: critical && placeholder });
  })).then(sortByWidth);
}
...
export function createURLPlaceholderSync (sources, getResources) {
  return sortByWidth(sources.map((source) => {
    const [{ url }, sizes] = getResources(source);
    return createSourceDescription(url, getMinMaxSize(sizes));
  }));
}
...
function getMinMaxSize (sizes) {
  const sortedSizes = Array.from(sizes).sort((a, b) => a.width > b.width ? 1 : -1);
