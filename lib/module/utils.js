...
function getOptions (options) {
  ...
  if ('maxIdleDuration' in options) {
    consola.warn(`[${pkg.name}] Option \`maxIdleDuration\` is deprecated, can be removed is no longer needed.`);
  }
  if ('maxIdleDurations' in options) {
    consola.warn(`[${pkg.name}] Option \`maxIdleDurations\` is deprecated, can be removed is no longer needed.`);
  }
