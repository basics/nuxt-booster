    ...
    Vue.directive('font', {
      bind (el, binding, vnode) {
        ...
        [].concat(binding.value).map((font) => {
          const isFontCritical = font.critical
          font.critical = vnode.context.isCritical || isFontCritical
      ...
      inserted (el, binding, vnode) {
        // has lazy fonts?
        const fonts = [].concat(binding.value).filter(font => !font.critical)
        if (fonts.length > 0) {
...
function updateNodeStyle (el, font) {
  Array.from(font.medias).forEach(media => font.getCSSSelectors(media).forEach(() => el.classList.add(...font.getClassNames())))
}
