  ...
  install (Vue, options) {
    ...
    Vue.mixin({
      ...
      head () {
        ...
        // script.push(preloadResources())
        const result = { link, __dangerouslyDisableSanitizers: ['style'] }
        if (style.length) {
...
function preloadResources () {
  return {
    hid: 'preload-resources',
    innerHTML: `
      document.querySelectorAll("[rel$='-resource']").forEach((item) => {
        console.log(item);
        item.rel = item.rel.replace('-resource', '');
      });
    `,
