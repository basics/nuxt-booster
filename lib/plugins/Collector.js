          ...
          .reduce((result, [key, value]) => {
            if (typeof value === 'function') {
              return Object.assign(result, {
                [key]: hydrateWhenVisible(() => value().then((component) => {
                  return wrapComponent(component.default, { critical: false })
                }), { observerOptions: { rootMargin: '0px' } })
              })
            } else if ('then' in value) {
              return Object.assign(result, {
                [key]: () => value.then(component => wrapComponent(component.default, { critical: true }))
              })
            } else {
              return Object.assign(result, { [key]: wrapComponent(value, { critical: true }) })
            }
          }, {})
...
function wrapComponent (component, data = {}) {
  return {
    render (h) {
      return h(component, { props: Object.assign({}, this.$attrs, data) })
    }
  }
}
