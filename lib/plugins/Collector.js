...
export default {
  install (Vue, options) {
    ...
    Vue.mixin({
      ...
      beforeCreate () {
        const lazyComponents = Object
          .entries((this.$options.speedkitComponents || {}).lazy || {})
          .reduce((result, [key, value]) => Object.assign(result, { [key]: hydrateWhenVisible(value, { observerOptions: { rootMargin: '0px' } }) }), {})

        const criticalComponents = Object
          .entries((this.$options.speedkitComponents || {}).critical || {})
          .reduce((result, [key, value]) => {
            return Object.assign(result, {
              [key]: () => value().then((component) => {
                component.default.props.critical = { type: Boolean, default: true }
                return component
              })
            })
          }, {})

        this.$options.components = Object.assign(this.$options.components, criticalComponents, lazyComponents)
      }
    })
  }
}
