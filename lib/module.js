...
const browserSupport = getBrowserslistConfiguration();
...
module.exports = async function (moduleOptions) {
  ...
  this.addTemplate({
    ...
    options: { ignorePerformance: options.ignorePerformance, performance: JSON.stringify(options.performance || {}), browserSupport }
  });
  this.addTemplate({
...
function getBrowserslistConfiguration () {
  if (pkg.browserslist) {
    return serialize({ regex: getUserAgentRegExp({ browsers: pkg.browserslist, allowHigherVersions: true, allowZeroSubversions: true }) });
  } else {
