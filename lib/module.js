...
const REQUIRED_NODE_MAJOR_VERSION = 14;

module.exports = async function (moduleOptions) {
  if (Number(process.versions.node.split('.')[0]) < REQUIRED_NODE_MAJOR_VERSION) {
    throw new Error(`nuxt-speedkit needs a node >= ${REQUIRED_NODE_MAJOR_VERSION}`);
  }
  ...
  this.nuxt.hook('modules:done', (moduleContainer) => {
    ...
    if (nuxtImageOptions && ['youtube', 'vimeo'].find(alias => !(alias in nuxtImageOptions.alias))) {
      consola.warn('For using `SpeedkitYoutube` and `SpeedkitVimeo` you have to set the required domains & aliases for the `IPX` in the `@nuxt/image` options. \nLearn more https://nuxt-speedkit.grabarzundpartner.dev/setup#nuxtimage');
    }
  });
