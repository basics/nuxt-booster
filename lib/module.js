...
function addBundleRendererDirective (bundleRenderer) {
  ...
  bundleRenderer.directives.font = function (vnode, binding) {
    const critical = binding.value.critical || vnode.context.isCritical

    binding.value.critical = critical
    // TODO: check dynamic class added to template domNode (:class) where the directive is defined
    vnode.data.staticClass = ([...(vnode.data.staticClass || '').split(' '), binding.value.getFamilyAsClassName(), ((critical && 'font') || '')].join(' ')).trim()
...
function getStyleString () {
  const style = [
    ...fonts.getFonts().map(font => font.toCSSFontFace()),
    ...fonts.getFontFamilies().map(family => family.getCSSDeclaration()),
    fonts.getCSSDeclaration()
  ]
