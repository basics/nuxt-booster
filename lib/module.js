...
module.exports = async function (moduleOptions) {
  ...
  if (!options.ignorePerformance && this.nuxt.options.ssr) {
  ...
  this.nuxt.options.alias[`${pkg.name}-components`] = join(this.nuxt.options.buildDir, pkg.name, 'components')

  const componentsDir = join(__dirname, 'components')
  const components = await getComponentFiles(componentsDir)
  components.forEach((file) => {
    this.addTemplate({
      src: join(componentsDir, file),
