...
function addBundleRendererDirective (bundleRenderer) {
  ...
  bundleRenderer.directives.font = function (vnode, binding) {
    let critical = vnode.context.$options.critical || (vnode.data.attrs && vnode.data.attrs.critical)
    if (vnode.componentInstance) {
      critical = vnode.componentInstance.$options.critical
    }
    critical = /^\s*$/.test(critical) || /^\s*(true|1|on)\s*$/i.test(critical) || critical

    vnode.context.$fonts.registerFont(binding.value, critical, vnode.context.$vnode.data.key)

    vnode.data.staticClass = ((vnode.data.staticClass || '') + ' ' + binding.value.getFamilyAsClassName()).trim()
    // if (('attrs' in vnode.data && 'critical' in vnode.data.attrs && vnode.data.attrs.critical !== false) || vnode.context.$options.critical) {
    if (critical) {
