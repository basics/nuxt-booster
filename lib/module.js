...
module.exports = async function (moduleOptions) {
  ...
  optimizieNuxtOptions(this.nuxt);
...
function optimizieNuxtOptions (nuxt) {
  nuxt.options.generate.manifest = false;
  nuxt.options.render.resourceHints = false;

  consola.info(`[${pkg.name}] \`nuxt\` options \`generate.manifest\` and \`render.resourceHints\` disabled for optimization.`);
}
