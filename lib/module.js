...
import { getPreloadsString } from './utils/preload'
...
module.exports = function (moduleOptions) {
  ...
  this.nuxt.hook('vue-renderer:ssr:templateParams', (page) => {
    ...
    page.HEAD += fonts && getPreloadsString(fonts)
    if (fonts) {
      fonts.reset()
    }
  })
  ...
  this.nuxt.hook('generate:page', (page) => {
    ...
    $('head link, body script').each((i, el) => {
      ...
      if (/\/[\d]*\.[\w]*(\.modern)?\.js/gm.test(src)) {
        console.log(src)
        const normSrc = src.replace(/\.[\w]*(\.modern)?\.js/, '')
        if (critical.length < 1) {
          critical.push(normSrc)
        }
        if (!critical.includes(normSrc)) {
          $(el).remove()
        }
      }
    })
...
function getStyleString () {
  if (fonts) {
    const style = [
      ...
      ...fonts.getFonts().map(font => font.getMediaWithClasses())
    ]
