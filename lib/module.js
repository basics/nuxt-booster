...
const templates = [
  // Components
  // 'components/LazyLink.vue',
  'components/LazyLink.js',
...
module.exports = function (moduleOptions) {
  ...
  // this.options.build.splitChunks = {
  //   layouts: false,
  //   pages: true,
  //   commons: false
  // }
  ...
  this.nuxt.hook('generate:page', (page) => {
    ...
    $('head link, body script').each((i, el) => {
    ...
    const list = []
    $('body script[type="module"]').each((i, el) => {
      list.push(el.attribs.src)
      $(el).remove()
    })

    $('body').append($(`<script type="module">window.addEventListener("load", () => ${JSON.stringify(list)}.forEach((value) => import(value)));</script>`))
